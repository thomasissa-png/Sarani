...
| H-03 | PiAPI est le provider optimal — ni Kling direct ni Kie.ai ne sont plus adaptés au volume Sarani | Changement de provider = update des routes API uniquement (impact faible si code derrière abstraction) | @ia | Avant Session A | **DÉCISION** — PiAPI + Kling 2.6 abandonné. Stratégie multi-modèle validée par Thomas (2026-03-27) : Veo 3.1 (Gemini API) pour previews client qualité max, Runway Gen-4 Turbo pour itérations rapides, Kling 3.0 (fal.ai) en fallback. APIs directes, pas d'agrégateur reverse-engineering. |
