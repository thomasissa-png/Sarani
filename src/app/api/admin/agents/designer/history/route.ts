...
import { designerHistorySchema } from "@/lib/validations/designer";

export async function GET(request: NextRequest) {
  try {
    const { searchParams } = new URL(request.url);

    const parsed = designerHistorySchema.safeParse({
    ...
    // Build conditions
    const conditions = [eq(agentOutputs.agentType, "designer")];
    ...
    console.error("Designer history error:", error);

    const message =
      error instanceof Error
        ? error.message
        : "Failed to fetch design history";
