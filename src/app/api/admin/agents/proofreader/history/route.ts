...
import { proofreaderHistorySchema } from "@/lib/validations/proofreader";

export async function GET(request: NextRequest) {
  try {
    const { searchParams } = new URL(request.url);

    const parsed = proofreaderHistorySchema.safeParse({
    ...
    // Build conditions
    const conditions = [eq(agentOutputs.agentType, "proofreader")];
    ...
    console.error("Proofreader history error:", error);

    const message =
      error instanceof Error
        ? error.message
        : "Failed to fetch proofreader history";
