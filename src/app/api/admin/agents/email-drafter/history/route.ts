...
import { emailDrafterHistorySchema } from "@/lib/validations/email-drafter";

export async function GET(request: NextRequest) {
  try {
    const { searchParams } = new URL(request.url);

    const parsed = emailDrafterHistorySchema.safeParse({
    ...
    // Build conditions
    const conditions = [eq(agentOutputs.agentType, "email-drafter")];
    ...
    console.error("Email Drafter history error:", error);

    const message =
      error instanceof Error
        ? error.message
        : "Failed to fetch email draft history";
