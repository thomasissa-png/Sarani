...
import { videoScriptHistorySchema } from "@/lib/validations/video-script";

export async function GET(request: NextRequest) {
  try {
    const { searchParams } = new URL(request.url);

    const parsed = videoScriptHistorySchema.safeParse({
    ...
    // Build conditions
    const conditions = [eq(agentOutputs.agentType, "video-script")];
    ...
    console.error("Video script history error:", error);

    const message =
      error instanceof Error
        ? error.message
        : "Failed to fetch video script history";
