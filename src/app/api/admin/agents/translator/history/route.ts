...
import { translatorHistorySchema } from "@/lib/validations/translator";

export async function GET(request: NextRequest) {
  try {
    const { searchParams } = new URL(request.url);

    const parsed = translatorHistorySchema.safeParse({
    ...
    // Build conditions
    const conditions = [eq(agentOutputs.agentType, "translator")];
    ...
    console.error("Translator history error:", error);
