...
import { copywriterHistorySchema } from "@/lib/validations/copywriter";
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Build conditions
    const conditions = [eq(agentOutputs.agentType, "copywriter")];
    ...
    console.error("Copywriter history error:", error);

    const message =
      error instanceof Error
        ? error.message
        : "Failed to fetch copywriter history";
