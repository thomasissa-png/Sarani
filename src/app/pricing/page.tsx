...
export default function PricingPage() {
  return (
    <div className="pt-[72px]">
      ...
      {/* Guarantee Strip */}
      <div className="border-y border-neutral-800 bg-surface-elevated">
      ...
      <Section ariaLabel="Pricing comparison">
        <h2 className="mb-10 text-center text-3xl font-bold text-brand-white sm:text-4xl">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[480px] border-collapse text-left">
            <thead>
              <tr>
                <th className="rounded-tl-lg border border-neutral-800 bg-surface-elevated px-6 py-4 text-sm font-bold uppercase tracking-wider text-neutral-500">
                  &nbsp;
                </th>
                <th className="border border-neutral-800 bg-brand-cerulean px-6 py-4 text-sm font-bold uppercase tracking-wider text-brand-black">
                  Sarani
                </th>
                <th className="rounded-tr-lg border border-neutral-800 bg-surface-elevated px-6 py-4 text-sm font-bold uppercase tracking-wider text-neutral-500">
                  Network Agency
                </th>
              </tr>
            </thead>
            <tbody>
              {COMPARISON_ROWS.map((row, i) => (
                <tr key={row.label}>
                  <td
                    className={`border border-neutral-800 bg-surface-elevated px-6 py-4 font-bold text-brand-white ${i === COMPARISON_ROWS.length - 1 ? "rounded-bl-lg" : ""}`}
                  >
                    {row.label}
                  </td>
                  <td className="border border-neutral-800 bg-surface-elevated px-6 py-4 text-brand-cerulean">
                    {row.sarani}
                  </td>
                  <td
                    className={`border border-neutral-800 bg-surface-elevated px-6 py-4 text-neutral-500 ${i === COMPARISON_ROWS.length - 1 ? "rounded-br-lg" : ""}`}
                  >
                    {row.agency}
                  </td>
                </tr>
...
function PricingCard({ category }: { category: PricingCategory }) {
  return (
    <div className="rounded-lg border border-neutral-800 bg-surface-elevated p-6 md:p-8">
      <h3 className="mb-5 text-xl font-bold text-brand-white">
        {category.title}
      </h3>
