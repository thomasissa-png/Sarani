...
15. **Audit "intention-first" obligatoire sur chaque page.** Avant de produire ou auditer une page, identifier l'intention du visiteur (découverte/évaluation/portfolio/qualification/action/éducation) et vérifier que le hero, les CTAs et la structure servent cette intention — pas une intention générique de conversion. Les CTAs commerciaux ("Start a project") ne doivent apparaître que sur les pages dont l'intention est ACTION (homepage hero, contact) ou en conclusion d'une page de conviction (About closing, Pricing closing). Sur les pages d'évaluation (Services, Work), d'éducation (Blog) ou de conviction (About), les CTAs sont déplacés en fin de parcours.
...
20. **Tests incrémentaux obligatoires avec scénarios réels.** Le patrimoine de tests GRANDIT AUTOMATIQUEMENT avec le projet. **Chaque QA DOIT inclure des scénarios end-to-end avec des données réelles** (pas mockées). Simuler les vrais payloads JSON (inbox emails, review items, feedback threads), pas des données parfaites inventées. Un QA qui dit "PASS" sans avoir testé un seul cas réel est un QA qui ne sert à rien. Signalé comme P0 sur 2 projets — les QA vérifiaient la structure (imports, types, tsc) mais PAS la logique métier réelle. À chaque bug signalé par Thomas, chaque fix appliqué, chaque feature ajoutée : @qa ou @fullstack DOIT ajouter un test unitaire correspondant AVANT de déclarer le travail terminé. Un fix sans test est un fix incomplet. @qa audite en vérifiant que le nombre de tests a augmenté proportionnellement aux changements de la session. Les tests ne sont jamais "terminés" — ils sont un patrimoine vivant. Signalé comme P0 sur 1 projet — Thomas passait trop de temps à tester manuellement des choses logiques que des tests auraient dû couvrir.
...
22. **Zéro modification de texte client-facing non sollicitée.** Ne JAMAIS renommer un label, un titre de section, un CTA, un texte visible par le client sans demande EXPLICITE de Thomas. Les "améliorations" de wording non demandées sont interdites. Si un agent ou un reviewer suggère un changement de label, le proposer à Thomas — ne pas l'appliquer. Signalé comme P0 sur 1 projet — "Creative Proposal" renommé en "Deliverables" sans demande, Thomas a dû corriger.
